...
class TestJobApplications:
    ...
    async def test_list_applications(self, client):
        ...
        response = await client.get(
            "/api/v1/marketplace/user/applications",
            headers={"Authorization": f"Bearer {TEST_USER_TOKEN}"}
        )
        ...
        if applications:
            ...
            response = await client.put(
                ...
                json={
                    ...
                    "interview_date": "2025-11-15T10:00:00"
                },
                headers={"Authorization": f"Bearer {TEST_USER_TOKEN}"}
            )
    ...
    async def test_get_application_stats(self, client):
        ...
        response = await client.get(
            "/api/v1/marketplace/user/application-stats",
            headers={"Authorization": f"Bearer {TEST_USER_TOKEN}"}
        )
...
class TestSavedJobs:
    ...
    async def test_list_saved_jobs(self, client):
        ...
        response = await client.get(
            "/api/v1/marketplace/user/saved-jobs",
            headers={"Authorization": f"Bearer {TEST_USER_TOKEN}"}
        )
    ...
    async def test_remove_saved_job(self, client):
        ...
        saved_jobs = list_response.json() if isinstance(list_response.json(), list) else list_response.json().get("saved_jobs", [])
        if saved_jobs:
            saved_id = saved_jobs[0]["id"]
            
            response = await client.delete(
                f"/api/v1/marketplace/saved-jobs/{saved_id}",
                headers={"Authorization": f"Bearer {TEST_USER_TOKEN}"}
            )
