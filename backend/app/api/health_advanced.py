...
async def check_database() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "healthy", "response_time_ms": round(duration * 1000, 2), "pool_stats": db_pool.get_stats()}
...
async def check_gemini_ai() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "unhealthy", "error": str(e), "api_configured": bool(settings.GEMINI_API_KEY)}
...
async def check_external_services() -> Dict[str, Any]:
    ...
    if settings.STRIPE_API_KEY:
        try:
            ...
            services["stripe"] = {"status": "healthy", "response_time_ms": round(duration * 1000, 2)}
        except Exception as e:
    ...
    if settings.SENDGRID_API_KEY:
        services["sendgrid"] = {"status": "configured", "api_key_present": True}
    else:
...
async def detailed_health_check():
    ...
    database_status, redis_status, ai_status, external_services = await asyncio.gather(
        check_database(), check_redis(), check_gemini_ai(), check_external_services(), return_exceptions=True
    )
    ...
    is_healthy = all(
        isinstance(svc, dict) and svc.get("status") in ["healthy", "disabled"] for svc in critical_services
    )
    ...
    return {
        ...
        "services": {
            "database": (
                database_status
                if not isinstance(database_status, Exception)
                else {"status": "error", "error": str(database_status)}
            ),
            "redis": (
                redis_status
                if not isinstance(redis_status, Exception)
                else {"status": "error", "error": str(redis_status)}
            ),
            "gemini_ai": (
                ai_status if not isinstance(ai_status, Exception) else {"status": "error", "error": str(ai_status)}
            ),
            "external": (
                external_services
                if not isinstance(external_services, Exception)
                else {"status": "error", "error": str(external_services)}
            ),
        },
