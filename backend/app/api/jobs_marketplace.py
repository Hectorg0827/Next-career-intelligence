...
async def get_user_preferences(user_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = client.table("user_job_preferences").select("*").eq("user_id", user_id).single().execute()
...
async def search_jobs(
    ...
    try:
        ...
        if query:
            query_builder = query_builder.or_(f"title.ilike.%{query}%," f"description.ilike.%{query}%")

        # Execute with pagination
        response = query_builder.order("posted_at", desc=True).range(offset, offset + limit - 1).execute()

        jobs = response.data if response.data else []

        return {"jobs": jobs, "total": len(jobs), "offset": offset, "limit": limit}
...
async def get_recommendations(
    ...
    try:
        ...
        try:
            goals_response = (
                client.table("career_goals").select("*").eq("user_id", user_id).eq("status", "active").execute()
            )
        ...
        jobs_response = (
            client.table("jobs").select("*").eq("status", "active").eq("is_spam", False).limit(200).execute()
        )
        ...
        logger.info(
            f"   Filters: skill_match≥{min_skill_match}%, distance≤{max_distance_km}km, goals={len(user_goals)}"
        )
...
@router.post("/apply")
async def apply_to_job(request: JobApplicationRequest, current_user: Dict = Depends(require_premium)):
    """
    ...
    try:
        ...
        # Get job details
        job_response = client.table("jobs").select("*, employers(*)").eq("id", request.job_id).single().execute()
        ...
        app_response = client.table("job_applications").insert(application_data).execute()

        application = app_response.data[0] if app_response.data else None

        # Update recommendation status
        client.table("job_recommendations").update({"status": "applied"}).eq("user_id", request.user_id).eq(
            "job_id", request.job_id
        ).execute()
...
async def get_my_applications(
    ...
    try:
        ...
        response = query_builder.order("submitted_at", desc=True).range(offset, offset + limit - 1).execute()
...
async def update_preferences(request: UpdatePreferencesRequest, current_user: Dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Upsert preferences
        response = (
            client.table("user_job_preferences").upsert({"user_id": current_user["user_id"], **update_data}).execute()
        )
...
async def clear_seeded_jobs():
    ...
    try:
        ...
        return {"success": True, "jobs_deleted": deleted_count, "message": f"Cleared {deleted_count} seeded jobs"}
