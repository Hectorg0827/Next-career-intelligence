...
async def get_career_forecast(user_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to forecast career paths: {str(e)}"
        )
...
async def get_early_warnings(user_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to scan for threats: {str(e)}"
        )
...
async def analyze_offer(request: OfferAnalysisRequest = Body(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to analyze offer: {str(e)}"
        )
...
async def get_peer_insights(user_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch peer data: {str(e)}"
        )
