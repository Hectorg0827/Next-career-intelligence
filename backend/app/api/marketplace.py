...
async def create_alert_preferences(
    ...
    existing = db.query(JobAlertPreferences).filter(
        JobAlertPreferences.user_id == current_user
    ).first()
...
async def get_matched_jobs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving matched jobs: {str(e)}"
        )
...
async def calculate_job_match(
    ...
    """
    try:
        result = await ai_matching_service.refresh_job_match(
            user_id=current_user,
            job_id=job_id,
            db=db
        )
