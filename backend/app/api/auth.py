...
async def verify_email(request: EmailVerificationRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Verify code
        code_record = await db_client.verify_code(
            email=request.email,
...
async def request_password_reset(request: PasswordResetRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if user:
            ...
            await db_client.create_reset_code(
                user_id=user['id'],
                email=request.email,
                code=reset_code
            )
...
async def reset_password(request: PasswordResetConfirm):
    ...
    try:
        ...
        reset_code_record = await db_client.verify_reset_code(
            email=request.email,
        ...
        await db_client.update_password(
            user_id=user['id'],
            password_hash=hashed_password
        )
