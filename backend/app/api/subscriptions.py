...
async def get_usage(user_id: str):
    ...
    try:
        ...
        sub_response = client.table('subscriptions')\
            .select('*')\
            .eq('user_id', user_id)\
            .eq('status', 'active')\
            .single()\
        ...
        analyses_response = client.table('analyses')\
        ...
        monthly_analyses_response = client.table('analyses')\
            .select('count', count='exact')\
            .eq('user_id', user_id)\
        ...
        return {
            ...
            "usage_percentage": {
                "analyses_monthly": min(100, (monthly_analyses_count / limits.get('analyses_per_month', 5)) * 100) if limits.get('analyses_per_month') != float('inf') else 0,
            }
        }
    ...
    try:
        ...
        if not price_id:
            raise HTTPException(
                ...
                detail=f"Invalid plan/billing combination: {price_key}"
            )
