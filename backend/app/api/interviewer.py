...
async def start_interview(request: StartInterviewRequest):
    ...
    try:
        ...
        questions = [InterviewQuestion(question=q.get("question", ""), user_response=None) for q in questions_data]
...
async def submit_answer(request: SubmitAnswerRequest):
    ...
    try:
        ...
        # 4. Build task prompt
        task_prompt = prompt_set["task"].format(question=question["question"], user_answer=request.answer)
        ...
        if client:
            try:
                client.table("interview_sessions").update(
                    {"questions": questions, "evidence_summaries": evidence_summaries}
