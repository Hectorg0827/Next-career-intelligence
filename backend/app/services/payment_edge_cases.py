...
stripe_circuit_breaker = get_circuit_breaker(
    name="stripe", failure_threshold=3, recovery_timeout=45, expected_exception=stripe.error.StripeError
)
...
class PaymentEdgeCaseHandler:
        ...
        try:
            ...
            if failure_code in ["card_declined", "insufficient_funds"]:
                ...
                await self._schedule_payment_retry(
                    user_id=user_id, payment_intent_id=payment_intent["id"], attempt_number=1
                )
    ...
    async def handle_duplicate_webhook(self, event_id: str) -> bool:
        ...
        try:
            ...
            result = await self.supabase.table("stripe_events").select("id").eq("event_id", event_id).execute()
            ...
            await self.supabase.table("stripe_events").insert(
                {"event_id": event_id, "processed_at": datetime.utcnow().isoformat(), "status": "processing"}
            ).execute()
    ...
    async def handle_webhook_processing_failure(self, event: Dict[str, Any], error: Exception) -> bool:
        """
    ...
    async def handle_subscription_downgrade_timing(self, user_id: str, from_plan: str, to_plan: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            await stripe_circuit_breaker.call(
                ...
                items=[{"id": subscription["items"]["data"][0]["id"], "price": self._get_price_id(to_plan)}],
            )
    ...
    async def handle_proration_dispute(self, user_id: str, subscription_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            proration_items = [item for item in invoice["lines"]["data"] if item.get("proration", False)]
    ...
    async def handle_refund_request(
        ...
        try:
            # Get payment intent
            payment_intent = await stripe_circuit_breaker.call(stripe.PaymentIntent.retrieve, payment_intent_id)
    ...
    async def handle_payment_dispute(self, dispute: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            await self._alert_payment_dispute(
                user_id=user_id, dispute_id=dispute["id"], reason=dispute_reason, amount=dispute_amount
            )
    ...
    async def _get_user_subscription(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = (
                await self.supabase.table("subscriptions")
                .select("*")
                .eq("user_id", user_id)
                .eq("status", "active")
                .single()
            ...
            if result.data:
                ...
                return await stripe_circuit_breaker.call(stripe.Subscription.retrieve, subscription_id)
            return None
        except Exception as e:
    ...
    async def _get_payment_failure_count(self, subscription_id: str) -> int:
        ...
        try:
            result = (
                await self.supabase.table("payment_events")
    ...
    def _get_user_friendly_failure_reason(self, failure_code: str) -> str:
        ...
        return error_messages.get(
            failure_code, "Your payment could not be processed. Please try again or use a different payment method."
        )

    async def _schedule_payment_retry(self, user_id: str, payment_intent_id: str, attempt_number: int) -> bool:
    ...
    async def _send_authentication_link(self, user_id: str, payment_intent_id: str) -> bool:
        ...
        try:
            ...
            await self.email_service.send_email(
                ...
                variables={"name": user.get("full_name", "User"), "auth_url": auth_url},
            )
            return True
        except Exception as e:
    ...
    async def _cancel_subscription_due_to_non_payment(self, subscription_id: str, user_id: str) -> bool:
        """Cancel subscription after multiple payment failures"""
        try:
            await stripe_circuit_breaker.call(stripe.Subscription.delete, subscription_id)
    ...
    async def _cancel_subscription_with_refund(self, subscription_id: str, user_id: str) -> bool:
        """Cancel subscription with refund"""
        try:
            await stripe_circuit_breaker.call(stripe.Subscription.delete, subscription_id)

            await self.supabase.table("subscriptions").update(
                {"status": "cancelled", "cancellation_reason": "refund", "cancelled_at": datetime.utcnow().isoformat()}
            ).eq("stripe_subscription_id", subscription_id).execute()
    ...
    async def _get_pending_invoices(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            invoices = await stripe_circuit_breaker.call(
                stripe.Invoice.list, customer=subscription["customer"], status="open"
            )
    ...
    async def _get_webhook_retry_count(self, event_id: str) -> int:
        ...
        try:
            result = (
                await self.supabase.table("stripe_events")
                .select("retry_count")
    ...
    async def _alert_payment_dispute(self, user_id: str, dispute_id: str, reason: str, amount: float) -> bool:
        ...
        try:
            ...
            capture_exception(
                ...
                {"dispute": {"user_id": user_id, "dispute_id": dispute_id, "reason": reason, "amount": amount}},
            )
            return True
        except Exception as e:
