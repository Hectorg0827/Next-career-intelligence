...
from app.models.orchestrator_schemas import OrchestratorOutput, JobOpportunity, AIDisplacementRiskOutput
...
class CareerOrchestrator:
    ...
    async def analyze_job_match(
        self, user_id: str, job: JobOpportunity, recent_conversation: Optional[str] = None
    ) -> OrchestratorOutput:
        """
        ...
        profile_update = await self._generate_profile_update(user_profile, job, recent_conversation)

        # Step 6: Info Requests for Coach
        info_requests = await self._generate_info_requests(user_profile)

        # Step 7: Warnings & Flags
        warnings = self._generate_warnings(user_profile, job, displacement_risk, compatibility_score)

        # Step 8: Internal Scores (for ranking)
        internal_scores = self._calculate_internal_scores(user_profile, job, compatibility_score, displacement_risk)
    ...
    async def _generate_profile_update(
        self, user_profile: UserProfile, job: JobOpportunity, recent_conversation: Optional[str]
    ) -> ProfileUpdate:
        """
        ...
        if recent_conversation:
            sentiment_insights = await self.sentiment_agent.analyze_conversation(user_profile, recent_conversation)
    ...
    def _generate_warnings(
        ...
        if user_profile.burnout_level and user_profile.burnout_level >= 7:
            ...
            user_hates = [s.description.lower() for s in user_profile.motivation_signals if s.signal_type == "hate"]
        ...
        for dealbreaker in dealbreakers:
            if "remote" in dealbreaker.preference.lower() and not job.is_remote:
                warnings.append(f"⚠️ This role is not remote, which conflicts with your stated dealbreaker preference.")
    ...
    def _calculate_internal_scores(
        ...
        risk_to_stability = {"Very Low": 95, "Low": 80, "Medium": 60, "High": 30}
        ...
        recommendation_score = int(0.4 * compatibility_score + 0.3 * stability_score + 0.3 * trajectory_score)
    ...
    async def rank_jobs(self, user_id: str, jobs: List[JobOpportunity]) -> List[Dict[str, Any]]:
        """
