...
ai_circuit_breaker = CircuitBreaker(failure_threshold=3, timeout=30, expected_exception=Exception)
...
class GeminiAIService:
    ...
    async def generate_content(
        self, prompt: str, use_cache: bool = True, cache_ttl: int = 3600, max_retries: int = 3
    ) -> str:
        """
        ...
        async def _generate():
            try:
                # Use circuit breaker
                response = ai_circuit_breaker.call(self.model.generate_content, prompt)
    ...
    async def generate_with_timeout(self, prompt: str, timeout: Optional[int] = None) -> str:
        ...
        try:
            return await asyncio.wait_for(self.generate_content(prompt), timeout=timeout)
    ...
    def get_circuit_breaker_status(self) -> dict:
        ...
        return {
            ...
            "last_failure": (
                ai_circuit_breaker.last_failure_time.isoformat() if ai_circuit_breaker.last_failure_time else None
            ),
        }
...
def with_ai_optimization(use_cache: bool = True, cache_ttl: int = 3600, max_retries: int = 3, timeout: int = 30):
    """
