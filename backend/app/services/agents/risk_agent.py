...
Be realistic and data-driven. Output ONLY valid JSON."""
    ...
    def _fallback_risk_assessment(self, job: JobOpportunity) -> AIDisplacementRiskOutput:
        ...
        low_risk_keywords = [
            "coach", "mentor", "counsel", "therapist", "nurse",
            "teacher", "trainer", "facilitator", "mediator",
            "manager", "leader", "director", "supervisor",
            "field", "on-site", "in-person", "physical"
        ]
        
        # Keywords indicating high automation risk
        high_risk_keywords = [
            "data entry", "transcription", "basic", "routine",
            "clerical", "administrative assistant", "receptionist"
        ]
    ...
    async def assess_current_job_risk(self, user_profile: UserProfile) -> Dict[str, Any]:
        ...
        if not user_profile.current_role:
            return {
                "risk_level": "Unknown",
                "justification": "No current role specified in profile."
            }
