...
class PeerBenchmarkingAgent:
    ...
    async def find_peer_insights(self, user_profile: UserProfile) -> Dict[str, Any]:
        """
    ...
    async def _find_similar_profiles(self, user_profile: UserProfile, max_results: int = 100) -> List[UserProfile]:
        """
        ...
        try:
            ...
            for profile_data in response.data:
                ...
                if abs(peer_years - years_exp) <= 3:
                    ...
                    if peer_role and (
                        current_role.lower() in peer_role.lower() or peer_role.lower() in current_role.lower()
                    ):
                        try:
    ...
    def _analyze_transitions(self, peers: List[UserProfile]) -> List[Dict[str, Any]]:
        ...
        for peer in peers:
            ...
            sorted_history = sorted(peer.work_history, key=lambda x: x.end_date or "9999", reverse=True)
    ...
    def _compare_salary(self, user_profile: UserProfile, peers: List[UserProfile]) -> Dict[str, Any]:
        ...
        if not peer_salaries or not user_salary:
            return {"data_available": False, "message": "Not enough salary data for comparison"}
    ...
    def _compare_skills(self, user_profile: UserProfile, peers: List[UserProfile]) -> List[str]:
