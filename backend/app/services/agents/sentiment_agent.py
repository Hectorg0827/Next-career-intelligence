...
from app.models.user_profile import UserProfile, MotivationSignal, UserPreference, PreferenceCategory, CareerGoal, RiskFactor
...
class SentimentAgent:
    ...
    def _build_sentiment_analysis_prompt(
        ...
        current_signals = [
            ...
            for s in user_profile.motivation_signals[:5]
        ]
...
Only extract what is CLEARLY stated or strongly implied. Don't invent. Output ONLY valid JSON."""
        ...
        try:
            ...
            if json_match:
                ...
                for signal_data in data.get("motivation_signals", []):
                    motivation_signals.append(MotivationSignal(
                ...
                for risk_data in data.get("risk_signals", []):
                    risk_signals.append(RiskFactor(
                        type=risk_data["type"],
    ...
    async def detect_job_rejection_patterns(
        self,
        user_profile: UserProfile
    ) -> List[str]:
        """
