...
class GapAgent:
    ...
    async def analyze_gaps(self, user_profile: UserProfile, job: JobOpportunity) -> Dict[str, Any]:
        """
        ...
        enriched_gaps, next_steps = await self._enrich_with_ai(user_profile, job, gaps)

        logger.info(f"Identified {len(enriched_gaps)} gaps for {job.title}")

        return {"skill_gaps_for_job": enriched_gaps, "next_steps_for_user": next_steps}

    def _identify_skill_gaps(self, user_profile: UserProfile, job: JobOpportunity) -> List[Dict[str, Any]]:
        """
        ...
        for required_skill in job.required_skills:
            ...
            if not has_skill:
                ...
                gaps.append({"skill_or_experience": required_skill, "severity": severity})

        return gaps

    def _determine_gap_severity(
        self, missing_skill: str, job: JobOpportunity, user_profile: UserProfile
    ) -> GapSeverity:
        """
    ...
    async def _enrich_with_ai(
        self, user_profile: UserProfile, job: JobOpportunity, gaps: List[Dict[str, Any]]
...
Be practical, specific, and actionable. Output ONLY valid JSON."""
    ...
    def _parse_gap_response(
        self, response_text: str, original_gaps: List[Dict[str, Any]]
    ) -> tuple[List[SkillGap], List[str]]:
    ...
    def _create_fallback_next_steps(self, user_profile: UserProfile, job: JobOpportunity) -> List[str]:
        ...
        if job.company:
            next_steps.append(f"Research {job.company}'s mission and values to show alignment in your application.")
