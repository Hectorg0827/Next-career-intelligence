...
class ProfileAgent:
    ...
    async def create_profile(self, user_id: str, email: Optional[str] = None) -> UserProfile:
        ...
        try:
            profile = UserProfile(
                user_id=user_id, email=email, created_at=datetime.utcnow(), last_updated=datetime.utcnow()
            )
    ...
    async def update_profile(self, user_id: str, profile_update: ProfileUpdate) -> UserProfile:
        ...
        try:
            ...
            for new_pref in profile_update.new_preferences_detected:
                if not any(
                    p.category == new_pref.category and p.preference == new_pref.preference for p in profile.preferences
                ):
            ...
            for need in profile_update.development_needs_detected:
                if not any(n.skill_or_experience == need.skill_or_experience for n in profile.development_needs):
