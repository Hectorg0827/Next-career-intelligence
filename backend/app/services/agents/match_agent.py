...
class MatchAgent:
    ...
    async def calculate_compatibility(self, user_profile: UserProfile, job: JobOpportunity) -> Dict[str, Any]:
        """
        ...
        compatibility_score = int(
            0.4 * skill_match + 0.3 * experience_alignment + 0.2 * preference_alignment + 0.1 * burnout_alignment
        )

        # Generate highlights using AI
        highlights = await self._generate_match_highlights(user_profile, job, compatibility_score)
    ...
    def _calculate_burnout_alignment(self, user_profile: UserProfile, job: JobOpportunity) -> int:
        ...
        hates = [signal for signal in user_profile.motivation_signals if signal.signal_type == "hate"]

        # Check what user enjoys
        enjoys = [signal for signal in user_profile.motivation_signals if signal.signal_type == "enjoy"]
    ...
    async def _generate_match_highlights(
        self, user_profile: UserProfile, job: JobOpportunity, compatibility_score: int
    ) -> List[str]:
        """
    ...
    def _build_highlights_prompt(self, user_profile: UserProfile, job: JobOpportunity, compatibility_score: int) -> str:
...
Output ONLY valid JSON array."""
    ...
    def _generate_fallback_highlights(
        ...
        if compatibility_score >= 80:
            highlights.append(
                f"Strong overall compatibility ({compatibility_score}/100) based on skills and experience."
            )
