...
class SupabaseClient:
    ...
    async def create_user(
        self, email: str, full_name: str, password_hash: str, is_verified: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    async def verify_email(self, user_id: str) -> bool:
        ...
        try:
            response = (
                ...
                .update({"is_verified": True, "updated_at": datetime.utcnow().isoformat()})
                .eq("id", user_id)
                .execute()
            )
    ...
    async def update_password(self, user_id: str, password_hash: str) -> bool:
        ...
        try:
            response = (
                self.client.table("users")
                .update({"password_hash": password_hash, "updated_at": datetime.utcnow().isoformat()})
                .eq("id", user_id)
                .execute()
            )
    ...
    async def create_verification_code(self, user_id: str, email: str, code: str) -> Dict[str, Any]:
        """
    ...
    async def mark_code_used(self, code_id: str) -> bool:
        ...
        """
        try:
            response = self.client.table("verification_codes").update({"is_used": True}).eq("id", code_id).execute()
    ...
    async def create_reset_code(self, user_id: str, email: str, code: str) -> Dict[str, Any]:
        """
    ...
    async def verify_reset_code(self, email: str, code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = (
                self.client.table("password_resets")
                .select("*")
                .eq("email", email)
                .eq("code", code)
                .eq("is_used", False)
    ...
    async def mark_reset_code_used(self, code_id: str) -> bool:
        ...
        """
        try:
            response = self.client.table("password_resets").update({"is_used": True}).eq("id", code_id).execute()
    ...
    async def save_onboarding_data(self, user_id: str, onboarding_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.client.table("onboarding").update(data).eq("user_id", user_id).execute()
    ...
    async def get_onboarding_progress(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            response = self.client.table("onboarding").select("*").eq("user_id", user_id).execute()
