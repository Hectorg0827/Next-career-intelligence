...
class JobSeeder:
    ...
    def __init__(self):
        ...
        self.job_categories = {
            ...
            "product": ["Product Manager", "Senior Product Manager", "Product Owner", "Technical Product Manager"],
            "design": ["UX Designer", "UI/UX Designer", "Product Designer", "Senior UX Researcher"],
            "marketing": [
                "Marketing Manager",
...
Make it realistic and specific to the role."""
            ...
            response = self.gemini.client.models.generate_content(
                model=self.gemini.model_name, contents=prompt, config=self.gemini.generation_config
            )
    ...
    async def _seed_employers(self) -> List[str]:
        ...
        for company in self.tech_companies:
            try:
                # Check if exists
                response = self.client.table("employers").select("id").eq("name", company["name"]).execute()
    ...
    async def clear_seeded_jobs(self):
        ...
        try:
            response = self.client.table("jobs").delete().eq("source", "seeded").execute()
