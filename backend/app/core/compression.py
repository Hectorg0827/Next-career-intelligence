    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if content_length:
            if int(content_length) > self.max_size:
                logger.warning(f"Request rejected: size {content_length} bytes exceeds limit {self.max_size} bytes")
                return Response(
                    content=json.dumps(
                        {
...
def get_compression_stats(original_size: int, compressed_size: int) -> dict:
    ...
    """
    if original_size == 0:
        return {"original_size": 0, "compressed_size": 0, "ratio": 0, "savings_percent": 0}
