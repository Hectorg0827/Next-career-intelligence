...
class BackgroundTaskManager:
    ...
    def add_task(self, func: Callable, trigger_type: str = "interval", **trigger_kwargs):
        """
        ...
        self.scheduler.add_job(self._wrap_task(func), trigger=trigger, id=func.__name__)
    ...
    def _wrap_task(self, func: Callable) -> Callable:
        ...
        async def wrapper():
            ...
            try:
                ...
                logger.info(f"Completed scheduled task: {task_name} " f"in {execution_time:.2f}s")
...
def setup_scheduled_tasks():
    ...
    task_manager.add_task(refresh_materialized_views_task, trigger_type="interval", hours=1)

    # Cleanup old data daily at 2 AM
    task_manager.add_task(cleanup_old_data_task, trigger_type="cron", hour=2, minute=0)

    # Run VACUUM ANALYZE weekly on Sundays at 3 AM
    task_manager.add_task(vacuum_analyze_task, trigger_type="cron", day_of_week="sun", hour=3, minute=0)

    # Clear expired cache every 30 minutes
    task_manager.add_task(clear_expired_cache_task, trigger_type="interval", minutes=30)

    # Health check every 5 minutes
    task_manager.add_task(health_check_task, trigger_type="interval", minutes=5)

    # Log performance metrics every hour
    task_manager.add_task(log_performance_metrics_task, trigger_type="interval", hours=1)
