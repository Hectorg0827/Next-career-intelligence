...
class SafetyGuardrails:
    ...
    AGENT_SCOPES = {
        AIAgentScope.RESUME_STUDIO: {
            "allowed": [
                "resume parsing", "cv analysis", "work history", "skills extraction",
                "job descriptions", "cover letters", "ats optimization", "career documents"
            ],
            "forbidden": [
                "general knowledge", "creative writing", "code generation",
                "personal advice", "medical advice", "legal advice", "financial advice"
            ]
        },
        AIAgentScope.CAREER_COACH: {
            "allowed": [
                "career guidance", "skill development", "goal setting", "career paths",
                "professional development", "job search strategy", "networking advice"
            ],
            "forbidden": [
                "therapy", "counseling", "personal relationships", "life coaching",
                "financial planning", "legal advice", "medical advice"
            ]
        },
        AIAgentScope.INTERVIEWER: {
            "allowed": [
                "interview practice", "behavioral questions", "star method",
                "technical questions", "role-specific questions", "feedback"
            ],
            "forbidden": [
                "personal questions", "discriminatory questions", "illegal questions",
                "sensitive attributes", "general conversation"
            ]
        }
    }
    ...
    def check_fake_credentials(text: str) -> Tuple[bool, List[str]]:
        ...
        for pattern in SafetyGuardrails.FAKE_CREDENTIAL_PATTERNS:
            ...
            if match:
                warnings.append(
                    f"Request involves potentially false credentials: '{match.group()}'"
                )
        
        if warnings:
            warnings.append(
                "I can only work with truthful, verifiable career information."
            )
...
class PrivacyFilter:
    ...
    def redact_pii(
        text: str,
        region: PrivacyRegion = PrivacyRegion.US,
