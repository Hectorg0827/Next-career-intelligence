...
class OrchestratorOutput(BaseModel):
    ...
    compatibility_score: int = Field(
        ...
        ge=0,
        le=100,
        description="Overall match score considering skills, preferences, and alignment"
    )
    
    match_highlights: List[str] = Field(
        default=[],
        description="Key reasons why this is a good match"
    )
    
    # Gaps & Growth
    skill_gaps_for_job: List[SkillGap] = Field(
        default=[],
        description="Missing skills or experience, with severity and advice"
    )
    
    next_steps_for_user: List[str] = Field(
        default=[],
        description="Concrete, short-term actions the user can take immediately"
    )
    ...
    info_request_for_coach: List[str] = Field(
        default=[],
        description="Questions the coach should ask to fill missing profile data"
    )
    
    # Internal Scores (for ranking/sorting)
    internal_scores: Optional[Dict[str, int]] = Field(
        default=None,
        description="Internal scores for backend use (stability, trajectory, etc.)"
    )
    
    # Warnings & Flags
    warnings: List[str] = Field(
        default=[],
        description="Red flags or cautions about this opportunity"
    )
