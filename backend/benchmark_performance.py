...
def benchmark_user_dashboard(user_id: str):
    ...
    result = (
        supabase.table("user_job_applications")
...
def benchmark_full_text_search(query: str):
    ...
    result = supabase.rpc("search_jobs_fulltext", {"search_query": query}).execute()
    return result
...
def benchmark_materialized_view_company_stats():
    ...
    result = supabase.table("mv_company_job_stats").select("*").order("total_jobs", desc=True).limit(100).execute()
    return result
...
def run_benchmark(name: str, func, *args, iterations: int = 10):
    ...
    return {"name": name, "avg": avg, "median": med, "min": min_time, "max": max_time}
...
def main():
    ...
    results.append(run_benchmark("Job Search (active + sorted)", benchmark_job_search, iterations=10))

    results.append(run_benchmark("User Dashboard", benchmark_user_dashboard, sample_user_id, iterations=10))

    results.append(run_benchmark("Company Job Listings", benchmark_company_listings, sample_company_id, iterations=10))

    results.append(run_benchmark("Salary Range Filter", benchmark_salary_range_filter, 80000, 150000, iterations=10))

    results.append(run_benchmark("Skill Matching", benchmark_skill_matching, ["Python", "SQL"], iterations=10))

    results.append(
        run_benchmark("Company Stats (Materialized View)", benchmark_materialized_view_company_stats, iterations=10)
    )
