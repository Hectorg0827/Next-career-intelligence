...
def test_health_endpoints():
    ...
    try:
        ...
        print(
            f"    Pool: {pool_stats.get('active_connections')}/{pool_stats.get('pool_size')} "
...
def test_caching_performance():
    ...
    print_test("Response Caching", passed, f"1st: {time1:.2f}ms | 2nd: {time2:.2f}ms | Speedup: {speedup:.2f}x")
...
def test_rate_limiting():
    ...
    print_test("Rate Limiting", rate_limited, f"{success_count} requests succeeded before rate limit")
...
def test_compression():
    ...
    try:
        ...
        r = requests.get(f"{BASE_URL}/api/jobs/suggest?q=engineer", headers={"Accept-Encoding": "gzip"})

        has_compression = "gzip" in r.headers.get("Content-Encoding", "")
        content_length = len(r.content)

        print_test("Response Compression", True, f"Compressed: {has_compression} | Size: {content_length} bytes")
    except Exception as e:
...
def test_database_optimizations():
    ...
    try:
        ...
        print_test(
            ...
            has_pool,
            f"Pool size: {pool_stats.get('pool_size')} | " + f"Available: {pool_stats.get('available_connections')}",
        )

        response_time = data.get("services", {}).get("database", {}).get("response_time_ms", 0)
        fast_response = response_time < 500

        print_test("Fast Database Response", fast_response, f"Response time: {response_time:.2f}ms")
